...
@section('content') 
  ...
  <div class="ERD">
    ...
    Each post has to option to edit or delete the post.  Editing the post is in a new page where the 
    ...
    their posts.  There is another page called Recent, which display every post from the last 7 days.
    ...
    Sanitisation, which are using single quotes, was used in the SQL query to get the posts were made in the last 7 
    days for dates between two variables.  Placeholders were used in many of the SQL queries, which the values that
    are to replace the placeholders are passed into an array in the DB::select().  Another measure that was taken is
    making sure that when submitting a form, the user hasn't left any blank input or textarea boxes.  This was done by 
    adding a required attribute that specifies that input and textarea fields cannot be empty.
